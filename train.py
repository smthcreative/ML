...
#
#
# dataset_path = os.path.join("Datasets", "CV")
#
# if not os.path.exists(dataset_path):
#     download_and_unzip("https://github.com/egorsmkv/cv10-uk-testset-clean/releases/download/v1.0/filtered-cv10-test.zip", extract_to="Datasets")

# dataset_path = "Datasets/LJSpeech-1.1"
# metadata_path = dataset_path + "/metadata.csv"
# wavs_path = dataset_path + "/wavs/"
metadata_path = 'Labels.xlsx'
wavs_path = 'Data/'
...
def normalize_labels(df):
    ...
    for index in df.index:
        normalized = str.lower(df['sentence'][index])
        for char in lst:
            normalized = normalized.replace(char, '')
        normalized_list = normalized.split(' ')
        normalized =  ' '.join([char.strip() for char in normalized_list])
        values.append(normalized)
    df.loc[:, 'sentence'] = values


metadata_df = pd.read_excel(metadata_path)
normalize_labels(metadata_df)


# metadata_df.columns = ["file_name", "transcription", "normalized_transcription"]
metadata_df = metadata_df[["id", "sentence"]]

# structure the dataset where each row is a list of [wav_file_path, sound transcription]
dataset = [[f"Data/{file}", label.lower()] for file, label in metadata_df.values.tolist()]
print(dataset[0])
