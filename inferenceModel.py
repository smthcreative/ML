...
if __name__ == "__main__":
    ...
    configs = BaseModelConfigs.load("Models/05_sound_to_text/202404252117/configs.yaml")

    model = WavToTextModel(model_path='Models/05_sound_to_text/202404252117/model.keras', char_list=configs.vocab, force_cpu=False)

    df = pd.read_csv("Models/05_sound_to_text/202404252117/val.csv").values.tolist()

    accum_cer, accum_wer = [], []
    for wav_path, label in tqdm(df):

        wav_path = 'Data/' + wav_path.split("/")[-1]
